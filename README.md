...
这是一个基于 ESP-IDF 的 ESP32-S3 项目，提供网页文件上传服务，并把上传到 LittleFS 中的 `bitmap.c` 图片数据解析后显示到 SSD1306 OLED 屏幕上。

## 功能

- ESP32-S3 连接 Wi-Fi 后启动 WebFS 网页文件管理器
- 支持在浏览器中上传、下载、重命名和删除文件
- 自动扫描 LittleFS 中的 `.c` 文件，解析 `oled_bitmap[]` 数组
- OLED 显示 64x128 bitmap 图片
- 按钮按下后切换显示下一张 bitmap 图片
...
每个有效 bitmap 文件对应一张图片。数组大小应为 1024 字节，对应 `64 x 128 / 8` 的单色 OLED 页面数据。
...
4. 在浏览器打开设备 IP，上传包含 `oled_bitmap[]` 的 `.c` 文件。
